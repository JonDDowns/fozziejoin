...
📂 [Benchmarking Scripts](https://github.com/JonDDowns/fozziejoin/tree/main/scripts)  
📊 [Benchmarking Outputs](https://github.com/JonDDowns/fozziejoin/tree/main/outputs)  
🔁 [Reproducible GitHub Workflow](https://github.com/JonDDowns/fozziejoin/blob/main/.github/workflows/run_all_benches.yml)
...
`qdapDictionaries::misspellings` as the source of noisy input. Each method is
tested at a sample size of 250.
...
params <- list(
  ...
  list(method = "levenshtein", mode = "inner", max_dist = 1)
)
...
Results are not guaranteed to be identical, especially for normalized distances.
For instance, this cosine distance join produces slightly different results:
...
Upon further exploration, `fozziejoin` contains 8 additional rows, each of
which has a cosine distance equal to the max distance. This is likely due to
rounding errors in floating point arithmetic.

```{r}
only_fozzie <- anti_join(fozzie_cos, fuzzy_cos) %>%
    mutate(dist = stringdist::stringdist(misspelling, word, method='cosine', q=3))
print(paste("Number of mismatched rows:", nrow(only_fozzie)))
print(paste("Max distance:", max(only_fozzie$dist)))

only_fuzzy <- anti_join(fuzzy_cos, fozzie_cos)
