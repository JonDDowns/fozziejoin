...
test_that("fozzie_difference_join returns a tibble", {
  ...
  result <- fozzie_difference_join(x, y)
  expect_s3_class(result, "tbl_df")
})

test_that("fozzie_distance_join returns a tibble", {
  x <- tibble(lat = c(47.6, 48.0), lon = c(-122.3, -122.5))
  y <- tibble(lat = c(47.7, 48.1), lon = c(-122.4, -122.6))
  result <- fozzie_distance_join(x, y)
  expect_s3_class(result, "tbl_df")
})
...
test_that("fozzie_interval_join returns a tibble", {
  ...
  result <- fozzie_interval_join(x, y)
  expect_s3_class(result, "tbl_df")
})
...
test_that("fozzie_temporal_interval_join returns a tibble", {
  ...
  result <- fozzie_temporal_interval_join(x, y)
  expect_s3_class(result, "tbl_df")
})
...
test_that("fozzie_temporal_join returns a tibble", {
  ...
  result <- fozzie_temporal_join(x, y)
  expect_s3_class(result, "tbl_df")
})
